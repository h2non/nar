...
var ncp, join, extract, download, chmodSync, resolve, EventEmitter, ref$, rm, mk, next, write, read, isWin, isArray, replaceEnvVars, isFile, isUrl, isDir, clone, extend, tmpdir, discoverPkg, defaults, install, apply, save, writeJson;
ncp = require('ncp').ncp;
...
ref$ = require('./utils'), rm = ref$.rm, mk = ref$.mk, next = ref$.next, write = ref$.write, read = ref$.read, isWin = ref$.isWin, isArray = ref$.isArray, replaceEnvVars = ref$.replaceEnvVars, isFile = ref$.isFile, isUrl = ref$.isUrl, isDir = ref$.isDir, clone = ref$.clone, extend = ref$.extend, tmpdir = ref$.tmpdir, discoverPkg = ref$.discoverPkg;
defaults = {
  gzip: true,
  dest: null,
  clean: true,
...
module.exports = install = function(options){
  ...
  createLink = function(binPath, dest, name){
    if (isWin) {
      return write(name + ".cmd", winBinScript(
      binPath));
    } else {
  ...
  createBinary = function(dest, path, name){
    ...
    binPath)) {
      if (global) {
        root = join(dest, '../../../', 'bin');
        createBinDir(root);
        return createLink(binPath, join(root, name), name);
      } else {
        root = join(dest, '../', '.bin');
        createBinDir(root);
        return createLink(binPath, join(root, name), name);
