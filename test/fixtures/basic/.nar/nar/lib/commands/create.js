...
var path, nar, program, ref$, echo, exit, onEntry, onError, exists, isDir, isFile, isString, options, create, normalize, apply;
path = require('path');
nar = require('../nar');
program = require('commander');
ref$ = require('./common'), echo = ref$.echo, exit = ref$.exit, onEntry = ref$.onEntry, onError = ref$.onError;
...
create = function(pkgpath, options){
  var debug, verbose, output, file, opts, onStart, onArchive, onEnd, create, e;
  ...
  create = function(){
    var archive;
    archive = nar.create(opts).on('start', onStart).on('error', onError(
    debug)).on('end', onEnd);
    if (debug || verbose) {
