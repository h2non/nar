...
var fs, fw, path, pack, utils, resolve, EventEmitter, dirname, basename, join, normalize, read, rm, tmpdir, clone, extend, copyBinary, keys, archiveName, isObject, isFile, isDir, isString, mk, stringify, vals, exists, checksum, lines, next, isArray, now, replaceEnvVars, discoverPkg, handleExit, once, isWin, narFile, ext, ignoredFiles, ignoreFiles, defaults, create, writeConfig, narManifest, includeFilesPatterns, getIgnoredFiles, isValid, outputFile, getFilename, applyPkgOptions, apply, resolvePkgPath, getBinaryPath, getModulePath, matchDependencies;
fs = require('fs');
fw = require('fw');
path = require('path');
pack = require('./pack');
...
read = utils.read, rm = utils.rm, tmpdir = utils.tmpdir, clone = utils.clone, extend = utils.extend, copyBinary = utils.copyBinary, keys = utils.keys, archiveName = utils.archiveName, isObject = utils.isObject, isFile = utils.isFile, isDir = utils.isDir, isString = utils.isString, mk = utils.mk, stringify = utils.stringify, vals = utils.vals, exists = utils.exists, checksum = utils.checksum, lines = utils.lines, next = utils.next, isArray = utils.isArray, now = utils.now, replaceEnvVars = utils.replaceEnvVars, discoverPkg = utils.discoverPkg, handleExit = utils.handleExit, once = utils.once, isWin = utils.isWin;
narFile = '.nar.json';
ext = 'nar';
...
module.exports = create = function(options){
  ...
  compressDependencies = function(dest, base, cb){
    var files, globals, addBinDirectory, getPkgPath, findPkg, calculateChecksum, definePkgInfo, doPack, compressDepPkgs, findGlobal, processGlobal, processDeps, dependenciesList, list;
    files = [];
    globals = [];
    ...
    doPack = function(options, done){
      if (options.name === '_modules-bindir') {
