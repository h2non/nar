...
var path, nar, program, ref$, echo, exit, exists, isFile, toKb, install;
path = require('path');
nar = require('../nar');
program = require('commander');
ref$ = require('../utils'), echo = ref$.echo, exit = ref$.exit, exists = ref$.exists, isFile = ref$.isFile, toKb = ref$.toKb;
program.command('install <archive>').description('\n  Install archive').usage('[archive] [options]').option('-o, --output <path>', 'Output directory. Default to node_modules').option('--user <user>', 'HTTP autenticantion user').option('--password <password>', 'HTTP user password').option('--proxy <url>', 'URL proxy to use').option('--timeout <number>', 'HTTP request timeout').option('--strict-ssl', 'Enable strict SSL').option('-d, --debug', 'Enable debug mode. More information will be shown').option('-v, --verbose', 'Enable verbose mode. A lot of information will be shown').on('--help', function(){
